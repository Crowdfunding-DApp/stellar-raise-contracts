//! Property-based and unit tests for `proptest_generator_boundary` validators.
//!
//! These are pure-function tests (no Soroban `Env` required), so they run fast
//! in CI. `PROPTEST_CASES` (set to 1000 in CI) controls how many cases each
//! `proptest!` property runs.
//!
//! ## Security Notes
//!
